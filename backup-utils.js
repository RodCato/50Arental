...
(function(root){
  ...
  const number=(v,p,{positive=false,integer=false}={})=>{if(!['number','string'].includes(typeof v)||String(v).trim()===''||!Number.isFinite(Number(v))||Number(v)<0||positive&&Number(v)<=0||integer&&!Number.isInteger(Number(v)))fail(p,'invalid number');return Number(v)};
