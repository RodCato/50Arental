...
(function(root){
  const buckets=['housing_recurring','utilities','housing_one_time','groceries','refundable_deposit','excluded','tax'];
  const categories=['Kitchen','Bathroom','Furniture','Utilities','Internet','Cleaning','Groceries','Security','Other'];
  const setupClasses=['move_in_essential','forgotten_essential','post_move_improvement','lease_service_setup','none'];
  const statuses=['kept','returned','refunded','reused','cancelled','avoided'];
  const statusLabels={kept:'Kept',returned:'Returned',refunded:'Refunded',reused:'Reused / owned ($0)',cancelled:'Cancelled',avoided:'Avoided / denied'};
  const receiptDefaults=category=>({category:categories.includes(category)?category:'Other',bucket:category==='Groceries'?'groceries':['Utilities','Internet'].includes(category)?'utilities':['Kitchen','Bathroom','Furniture','Cleaning','Security'].includes(category)?'housing_one_time':'excluded',setupClass:'none'});
  ...
  function expenses(items){
    const totals={expenses:0,housing_recurring:0,utilities:0,housing_one_time:0,groceries:0,tax:0,other:0,refunds:0,avoided:0};
    for(const i of items){
      ...
      if(['housing_recurring','utilities','groceries','tax'].includes(i.bucket))totals[i.bucket]+=net;
  ...
  const api={buckets,categories,setupClasses,statuses,statusLabels,receiptDefaults,netItem,depositReturned,depositExpense,expenses,benchmark};
