...
**Local mode remains the default. Phase 2B adds explicit, owner-bound cloud activation on each device; see [activation, offline behavior and rollback](CLOUD-CUTOVER.md).** Nothing uploads on sign-in for local-mode devices. Activated devices replay their explicitly queued mutations after authentication. Local rollback retains IndexedDB images, local import/export and legacy Google controls. Cloud mode uses its owner-bound repository and private photo workflow. Google Drive sync is **LEGACY / pending retirement**; do not treat it as the new multi-device architecture.

Architecture: static PWA → separate Supabase Auth/client boundary → owner-protected Postgres tables and private evidence Storage. `cloud/client.mjs` owns client/session/Auth; `cloud/data.mjs` only reads connectivity metadata; `cloud/ui.mjs` connects the small Settings panel. Cloud-authoritative structured writes and private evidence use the authenticated repository/RPC boundary. No OCR or Alexa endpoint is enabled. The separately invoked [certified Phase 2A importer](MIGRATION.md) creates a verified cloud copy without changing application behavior.
...
Sessions use the dedicated `50a-supabase-auth` localStorage key with `persistSession: true` and `autoRefreshToken: true`. They survive reload, tab/window close and browser/PWA restart on the same origin and browser profile. Supabase handles refresh-token rotation; ordinary access-token expiry does not require email. Magic Link implicit callback detection remains enabled and consumes/clears fragment credentials. Old sessionStorage credentials are not copied: one fresh Magic Link may be required after this deployment. Separate browser/PWA storage profiles require their own initial sign-in.

Cloud startup and Auth changes verify the user with `getUser()` before unlocking the owner-bound cache. A cold offline start remains locked until identity can be verified; the app does not trust a cached session's user object alone. Sign out has local-device/browser scope, removes persistent Auth credentials, locks personal data and preserves ledger/recovery material. Other devices remain signed in. Clearing site data or a revoked/invalid refresh session requires sign-in again.

Auth credentials remain separate from portable v3/Google backup payloads, cloud caches and business tables. The service worker only handles same-origin shell files; it does not cache Supabase Auth responses. Run `npm run test:auth:browser` with Playwright available (or `PLAYWRIGHT_MODULE` and `CHROME_EXECUTABLE` overrides) for real-SDK synthetic callback, refresh, persistent-profile restart, sign-out and account-isolation checks. Tests send no production email or business writes.
...
The migration creates private bucket `50a-evidence`, limited to WebP/JPEG/PNG and 20 MiB per file. Paths are `{owner_uuid}/receipts/{attachment_uuid}.webp` or `{owner_uuid}/property/{attachment_uuid}.webp` (JPEG/PNG extensions also accepted). Four Storage object policies allow access only under the authenticated owner's first path component; writes also validate folder/filename shape. Phase 3 uses authenticated private uploads/downloads and no public or persisted signed URLs. Existing local images are not automatically migrated. Review any pre-existing Storage policies before applying: permissive policies on the same bucket combine with OR, so unrelated broad policies can undermine isolation.
...
### Cloud photos (Phase 3)

See [Cloud evidence](CLOUD-EVIDENCE.md) for private photo capture, recovery/cleanup, complete portable backup, isolated test coverage and the post-deploy phone/Mac verification procedure. New cloud photo operations require a connection; local rollback evidence remains in IndexedDB.

## Phase 4A — receipt vision proof of concept

Saved cloud receipt images can be analyzed from the transaction card's **Analyze receipt** button. The review dialog has image selection, explicit ordering and a same-receipt confirmation for multiple images. One intentional Analyze/Retry click makes at most one OpenAI request. It never creates or updates transactions, items, categories, evidence, settings or mutation queues. There is no Apply/Save action. Results live only in the dialog's memory and clear on close, sign-out, account change or refresh; Backup v3 remains unchanged.

Architecture: browser → `POST /api/receipt-ocr` → Supabase Auth `getUser(bearer)` → owner-scoped receipt metadata and parent transaction → private Storage download → official OpenAI SDK `responses.create()` → strict validated extraction → read-only review. The root `api/receipt-ocr.mjs` is a Vercel Node function; `server/` is never imported by the browser or copied to `dist`. A plain Python/static development server cannot execute this route. Vercel builds the static shell and the root API function separately; the function has a 60-second duration limit.

Configuration:

- Existing **server-only** `OPENAI_API_KEY` in Vercel Production and Preview. Do not recreate it, pull environment files, expose it through public config, or use a `NEXT_PUBLIC_` key. No key value is needed in this repository.
- Optional **server-only** `OPENAI_RECEIPT_MODEL`. No new variable is required; the default is `gpt-6-luna`, selected from the [current model catalog](https://developers.openai.com/api/docs/models/gpt-6-luna) for efficient image understanding, Responses and Structured Outputs support. Account access to this model still needs the explicit live test. Environment changes require a new deployment.
- Existing public Supabase URL and publishable/anon key must be available to the server as well as the static build, through the existing `SUPABASE_*` / `NEXT_PUBLIC_SUPABASE_*` allowlist. No Supabase service-role key is used.

The browser supplies only `{attachment_ids:[...]}` and its normal bearer token. The server verifies identity with [Supabase Auth getUser](https://supabase.com/docs/reference/javascript/auth-getuser), then checks every selected row's owner, receipt type, same transaction, owner/parent relationship, private bucket, exact owner receipt path, MIME and size. Supabase reads use that user's token and RLS. Missing/other-owner/property evidence receives a safe denial. Client-provided owners, models, URLs, keys and paths are rejected. Only JPEG, PNG and WebP are accepted, with byte signatures checked after download. Limit: 1–4 distinct images, 20 MiB combined. No database or Storage write operation exists in the endpoint.

[Responses Structured Outputs](https://developers.openai.com/api/docs/guides/structured-outputs) uses `text.format` with `json_schema`, `strict:true`, all fields required and `additionalProperties:false`. Receipt fields: nullable merchant, date, time, currency, subtotal, tax, tip, discounts and total; purchased items with description, nullable quantity/unit price/total price/SKU/raw text; warnings; null confidence. There is no native calibrated confidence, so item and overall confidence are always null. Server validation rejects malformed/incomplete/refused output. Invalid or ambiguous dates are null, with warnings and no current-date fallback. Visible money is preserved even when arithmetic does not reconcile. No categorization is requested or produced. Image text is explicitly untrusted data, never task instructions; displayed results use text nodes, not HTML.

Images are ordered portions of **one receipt**; the prompt asks the model to avoid duplicate overlapping sections without collapsing separate repeated purchases. Users must confirm this for multiple selected images. These safeguards cannot guarantee perfect overlap detection or OCR accuracy. Stored compressed bytes are sent without another recompression pass. [Image detail](https://developers.openai.com/api/docs/guides/images-vision) is `high` for small receipt text, with the existing approximately 1600px/quality .82 evidence compression and four-image bound limiting input size. Images discarded during earlier compression cannot regain lost detail.

Privacy: pressing Analyze explicitly sends selected receipt images to OpenAI. Requests use `store:false` to avoid retaining Responses application state; this **does not promise zero retention** or change organization settings. OpenAI's applicable [data controls and abuse-monitoring policies](https://developers.openai.com/api/docs/guides/your-data) still apply. Images, tokens and extracted bodies are not logged by application code. Response headers disable browser/CDN caching; shell cache `50a-ledger-shell-v21` ignores the endpoint, private evidence and external APIs. Model and input/output/total token counts are displayed in a disclosure when available; no dollar charge is estimated. Usage is not persisted.

Timeouts and failures are sanitized: auth, missing evidence, unsupported/damaged MIME, size limits, metadata/download errors, quota/credits, rate limits, key/model availability, refusal and invalid/incomplete output. Provider retries are disabled. Each Supabase request is bounded to 10 seconds, the total server deadline is 55 seconds, and OpenAI has a 45-second timeout. UI requests time out at 65 seconds. A timeout or closing the dialog does not guarantee provider cancellation or zero cost; retry only intentionally. The Analyze button and selection controls are disabled during a request. The POC has no global distributed rate limiter; use a controlled owner-led evaluation and existing OpenAI usage monitoring.

Validation uses synthetic fixtures and mocked OpenAI responses, with **no paid/live API requests and no production evidence**. `npm test` includes endpoint authorization/error/schema, official SDK mocked transport, secret-boundary and service-worker tests. `npm run test:receipt:browser` covers desktop and 390px phone review, order/confirmation, explicit retry, HTML injection safety, no state persistence/writes and sign-out/late-response cleanup. Existing evidence browser tests cover the real saved-transaction entry point, private thumbnails, unchanged cloud snapshot/operation receipts and all prior evidence workflows. Tests can set `PLAYWRIGHT_MODULE` and `CHROME_EXECUTABLE` for a locally installed browser.

### Manual real-receipt evaluation after deployment

1. Open the intended deployed Preview (or Production after an approved merge) and sign in as the cloud ledger owner. Refresh once to load shell v21.
2. Choose an existing saved transaction with receipt evidence. Press **Analyze receipt** on its card to open review; opening alone sends nothing to OpenAI.
3. Select a readable image. For a long receipt, select up to four portions, reorder them and confirm they belong to the same receipt. Press **Analyze receipt** inside the dialog once.
4. Compare merchant, date/time, each item, quantities, visible prices, tax and total with the original. Check unclear dates are null, mismatches remain visible, overlapping images do not duplicate items, and warnings are useful. Expand Model and usage to inspect token counts. Use Retry only intentionally after a failure.
5. Close the dialog and verify the saved transaction, dashboard, pending changes and conflicts remain unchanged. Reopen review to confirm results were not persisted. Repeat deliberately on Mac and phone with readable, faded/ambiguous and long/overlapping receipts.
6. Report extraction accuracy and failures without sharing private image bytes or unrelated ledger contents. This manual test is the only live-API step; it has not been performed by automated tests. **Phase 4B transaction-draft integration is not cleared until real receipt accuracy has been evaluated.**

## Phase 4B — reviewed receipt transaction drafts

Phase 4B supersedes the Phase 4A read-only result panel with an editable, **unsaved** review. Analysis still requires an existing saved transaction with 1–4 private receipt images. The server remains read-only. `cloud/receipt-draft.mjs` is a pure review/mapping layer that can later support a new-transaction scan flow without coupling OCR to persistence; category-learning is not implemented.

`finance-utils.js` is the shared canonical contract for transaction categories, financial buckets, setup classes and statuses. It feeds normal transaction dropdowns, review dropdowns, strict server suggestion enums and backup/cloud validation where applicable. Categories remain Kitchen, Bathroom, Furniture, Utilities, Internet, Cleaning, Groceries, Security and Other—there is no Household category. Existing free-text category values in historical cloud/backup records remain valid; tightening those would damage compatibility. Setup classes remain separate from financial buckets. The existing `excluded` bucket still contributes to other expenses unless status makes the amount zero; avoided/cancelled/reused status semantics are unchanged.

The model now suggests category, bucket and setup class independently per merchandise item using enum-constrained nullable fields. Unknown suggestions fall back to Other/excluded/none in review. Never classify an entire mixed receipt from its merchant. Category selection offers a canonical default bucket; both bucket and setup class can be overridden under the item's disclosure. Setup class is applied only for housing_one_time. Suggestions are labeled **AI suggested** until edited; bulk selection, Select all, Clear selection and Assign selected support large receipts. Users can edit merchant/date/checkpoints, descriptions, charged totals, quantities and classifications, remove items or add missing items. Technical metadata, original unmodified extraction, warnings and token usage remain in collapsed details. Quantity is reference data, not a multiplier: line totals already represent the charged amount. No quantity/SKU/raw-text fields are added to the permanent financial schema.

### Approved sales-tax and savings semantics

The new `tax` financial bucket contains an explicit positive **Sales tax** line: category Other, bucket tax, setupClass none, status kept, actual tax amount, zero adjustment. Review identifies it as a system-generated component and lets the user correct the amount without categorizing it. A verified zero tax produces no extra zero-value line; unreadable/blank tax requires user verification, including entering 0 when appropriate. Purchased-item prices are never inflated or allocated proportionally for tax.

Tax contributes to transaction totals and total actual expenses, with its own monthly metric/filter and all-period filter totals. It does not contribute to groceries, utilities, housing/setup, refundable deposits or the Josh housing benchmark/run-rate/savings. The overview total expenses already includes it. Tax remains separate from Other in monthly subtotals. Legacy inferred setup classifications and move-in housing calculations do not absorb tax.

Receipt savings/discounts are **informational only**. Charged post-discount prices remain unchanged; savings never populate adjustment/refunds, gift-card offsets, one-time amounts, special statuses or a negative item. Reconciliation is reviewed merchandise + tax versus receipt total, using integer cents. Example: $79.46 + $5.30 = $84.76, difference $0.00, regardless of $12.96 informational savings. A discrepancy/unreadable total requires acknowledgement before Apply; no balancing item is invented. If extraction used pre-discount rather than charged prices, the user must correct those amounts explicitly. Tip/time/currency and other reference fields remain visible/editable in details; tips are not silently applied, and any needed extra financial line must be reviewed manually. Non-USD currency is blocked; there is no conversion feature.

Apply checks that the source transaction has not changed, requires explicit confirmation to replace its existing draft items, and opens the normal transaction editor. Settings-managed transactions cannot be replaced. Reviewed merchant is used only when non-empty; a blank OCR date preserves the existing date. Attachments, notes and transaction identity remain intact. Receipt total is a checkpoint, never an extra total line. **Apply performs no ledger/Storage/queue write.** Cancel discards the form draft. Only normal Save uses the existing atomic cloud path, and only then does the changed transaction propagate across devices. OCR/review state never enters Backup v3 or server storage.

### Tax migration and release order

`supabase/migrations/20260926000100_receipt_tax_bucket.sql` expands only `transaction_items_bucket_check`, validates it and commits atomically. It changes no existing rows, RPC code, RLS, ownership, Storage or privileges. Disposable PostgreSQL tests dry-run it with rollback, apply it, compare all existing rows/timestamps, and exercise tax writes and invalid-bucket rejection through the existing atomic RPC. **It has not been applied to production by development/tests.**

A reviewed deployment of this migration must precede saving tax-bearing transactions. Then deploy/merge the application, refresh both Mac and phone to shell v22, and verify both use the new build before saving tax records. Backups remain v3: tax is an allowed bucket on ordinary line items and uses existing hashes/serialization/restore semantics. Older validators/clients do not recognize tax-bearing records; upgrade them rather than altering or stripping tax from backups. Existing tax-free v3 backups continue to validate unchanged. The historical Phase 2A schema certificate remains unchanged; it describes that historical schema, not the new tax constraint.

Automated tests use a sanitized synthetic mixed receipt with repeated purchases, an excluded-void warning, charged prices, tax, informational savings and unreadable date. They mock OpenAI; no real receipt or private purchase data is committed. Review browser tests cover desktop/390px and 32 items, bulk edits/add/remove, replacement cancellation, explicit retry, safe text rendering and zero persistence. Integrated real SDK/local PostgreSQL tests prove Apply/cancel makes zero operations and normal Save creates one atomic operation; receipt bytes/metadata remain intact, tax filters reconcile and cloud Backup v3 validates. Unit tests verify independent raw extraction, enum rejection, tax math, date/amount guards, unchanged benchmark and local/cloud/backup round-trips.

### Physical test after deployment (initial pass stops before Save)

1. With the migration and app deployed, refresh Mac and phone to v22. Open the existing Walmart transaction and analyze the same saved receipt explicitly.
2. Verify extraction matches the physical receipt: all purchased items, repeated items, void exclusion, charged prices, date warnings, subtotal, tax and total. Confirm savings are informational and not subtracted again.
3. Correct at least one category; select several items and bulk-assign Groceries; remove an item and add a missing/test item. Verify descriptions and amounts and that modified controls say User changed.
4. Correct Sales tax if needed. Review merchandise + tax, receipt total and remaining difference. Acknowledge any deliberate test discrepancy. Leave unreadable date blank to retain the original.
5. Press Apply to transaction and explicitly confirm replacement. Inspect the normal editor: merchant, original/entered date, merchandise classifications, exactly one Sales tax line when positive, and original attachments/notes. **Do not Save on this first pass.**
6. Cancel the transaction editor. Verify the cloud ledger, dashboard, pending changes/conflicts, attachments and the other device are unchanged.
7. Repeat on the other device. Only with explicit user approval should a later test Save an appropriate/disposable transaction; then verify one atomic update, cross-device totals, separate tax and a valid Backup v3. No such production Save is performed by development/CI.
