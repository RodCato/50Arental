-- Expand property evidence to zero or many photos. Preserve all IDs, paths and bytes.
-- Stable per-property ordering; old single-photo rows receive position zero.
begin;
alter table public.attachments add column property_position integer not null default 0 check (property_position >= 0);
